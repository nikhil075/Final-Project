...
mod issue_id_card {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("nft_id", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let e_id: u32 = Default::default();
		let employee_name: String = Default::default();
		let emp_address: String = Default::default();
		let emp_email: String = Default::default();
		...
		let (id_pubkey, _) = Pubkey::find_program_address(
			&[b"id", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(id_pubkey, false),
		...
		let data = NftIdInstruction::IssueIdCard(crate::generated::instructions::IssueIdCardArgs{
			e_id,
			employee_name,
			emp_address,
			emp_email,
